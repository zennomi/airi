...
import { useDevicesList, useUserMedia, useWebWorker } from '@vueuse/core'
...
interface EventLoading {
  status: 'loading'
  data: string
}

interface EventInitiate {
  status: 'initiate'
  name: string
  file: string
  // Not used
...
type MessageEvents = EventLoading | EventInitiate | EventDownload | EventProgress | EventDone | EventReady | EventStart | EventUpdate | EventComplete
type ProgressMessageEvents = EventInitiate | EventProgress | EventDone

const { post, data } = useWebWorker<MessageEvents>(Worker, { type: 'module' })
const { audioInputs } = useDevicesList({ constraints: { audio: true }, requestPermissions: true })
const constraints = reactive({ audio: { deviceId: '' } })
      ...
      <div class="scrollbar-thin relative h-full flex flex-col items-center justify-center overflow-auto">
        ...
        <div class="flex flex-col items-center px-4">
          ...
          <div v-if="status === 'ready'" class="relative w-full flex flex-col justify-center">
            <div v-for="input of audioInputs" :key="input.deviceId">
              <input :id="input.deviceId" v-model="constraints.audio.deviceId" type="radio" name="audioInput" :value="input.deviceId">
              <label :for="input.deviceId">{{ input.label }}</label>
            </div>
          </div>
